...
await describe('Unique Timed Entry Queue - Export Operations', async () => {
    ...
    await it('does toArray()', () => {
        ...
        assert.deepStrictEqual(queue.toArray(), ['entry1', 'entry2', 'entry3'], 'toArray should return correct entries');
    });
    await it('doesPendingToArray()', () => {
        ...
        queue.pendingToArray().toSorted(), ['entry1', 'entry2', 'entry3'], 'pendingToArray should return correct pending entries');
    });
});
