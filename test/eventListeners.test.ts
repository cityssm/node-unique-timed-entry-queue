...
await describe('Unique Timed Entry Queue - Event Listeners', async () => {
  ...
  await it('dispatches "enqueue" event', async () => {
    queue = new UniqueTimedEntryQueue<string>(100)
    ...
    assert.strictEqual(
      eventDispatched,
      true,
      '"enqueue" event should have been dispatched'
    )
  })
  ...
  await it('adds and removes event listeners', () => {
    ...
    assert.strictEqual(
      eventDispatched,
      false,
      '"enqueue" event should not have been dispatched after listener removal'
    )
  })
})
