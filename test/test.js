...
await describe('Unique Timed Entry Queue', async () => {
    ...
    await it('should clear pending entries correctly', () => {
        ...
        assert.strictEqual(queue.pendingSize(), 1, 'There should be 1 pending entry after clearing "entry1"');
        debug('Clearing all pending entries');
        queue.clearPending();
        assert.strictEqual(queue.pendingSize(), 0, 'There should be no pending entries after clearing all');
    });
    await it('should handle immediate enqueueing with zero delay', () => {
        const queue = new UniqueTimedEntryQueue(0); // 0 seconds delay
        debug('Enqueuing "entry1" and "entry2" with zero delay');
        queue.enqueue('entry1');
        queue.enqueue('entry2');
        assert.strictEqual(queue.size(), 2, 'Queue should have 2 entries immediately after enqueueing with zero delay');
        assert.strictEqual(queue.isEmpty(), false, 'Queue should not be empty immediately after enqueueing with zero delay');
        assert.strictEqual(queue.pendingSize(), 0, 'There should be no pending entries after enqueueing with zero delay');
        const dequeuedEntry1 = queue.dequeue();
        const dequeuedEntry2 = queue.dequeue();
        assert.strictEqual(dequeuedEntry1, 'entry1', 'First dequeued entry should be "entry1"');
        assert.strictEqual(dequeuedEntry2, 'entry2', 'Second dequeued entry should be "entry2"');
    });
    await it('should allow custom delay per entry', async () => {
        const queue = new UniqueTimedEntryQueue(10_000); // Default 10 seconds delay
        debug('Enqueuing "entry1" with 2 seconds delay and "entry2" with 4 seconds delay');
        ...
        await wait(3000);
        assert.strictEqual(queue.size(), 1, 'Queue should have 1 entry after 3 seconds');
        assert.strictEqual(queue.pendingSize(), 1, 'There should be 1 pending entry after 3 seconds');
        const dequeuedEntry1 = queue.dequeue();
        assert.strictEqual(dequeuedEntry1, 'entry1', 'Dequeued entry should be "entry1"');
        /*
        ...
        await wait(2000);
        assert.strictEqual(queue.size(), 1, 'Queue should have 1 entry after total 5 seconds');
        assert.strictEqual(queue.pendingSize(), 0, 'There should be no pending entries after total 5 seconds');
        const dequeuedEntry2 = queue.dequeue();
        assert.strictEqual(dequeuedEntry2, 'entry2', 'Dequeued entry should be "entry2"');
    });
    ...
    await it('should handle different data types as entries', async () => {
        ...
        for (const expectedEntry of entries) {
            ...
            assert.deepStrictEqual(dequeuedEntry, expectedEntry, `Dequeued entry should match the enqueued entry: ${JSON.stringify(expectedEntry)}`);
        }
    });
});
