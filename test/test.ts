...
await describe('Unique Timed Entry Queue', async () => {
  ...
  await it('should enqueue all pending entries immediately on enqueuePending', () => {
    ...
    assert.strictEqual(
      queue.size(),
      0,
      'Queue should be empty before calling enqueuePending'
    )
    ...
    assert.strictEqual(
      queue.pendingSize(),
      0,
      'There should be no pending entries after calling enqueuePending'
    )

    assert.strictEqual(
      queue.size(),
      2,
      'Queue should have 2 entries after calling enqueuePending'
    )
  })
